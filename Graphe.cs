...
namespace Problème_scientifique_informatique
{
    public class Graphe
    {
        ...
        public Graphe(int[,] matrice_relation)
        {
            this.matrice_relation = matrice_relation;
        }


        public int OrdreDuGraphe(int[,] matrice_relation)         /// L'ordre du graphe est le nombre de sommet qu'il contient
        {
            int ordre = 0;
            for(int i=0; i<matrice_relation.GetLength(0); i++)
            {
                ...
                {
                    if (matrice_relation[i,j]>ordre)                        
                    {
                        ordre=matrice_relation [i,j];               /// l'ordre sera donc le nombre le plus élevé de la matrice_relation, extraite du fichier
                    }
                }
            }
        ...
        public int TailleDuGraphe(int[,] matrice_relation)     /// La taille du graphe est le nombre d'arêtes qu'il contient soit le nombre de lignes de la matrice_relation
        {
        ...
        public List<int>[] GenererListeAdjacence(int[,] matrice_relation, int ordre)
        {           
            ...
            for (int i = 0; i < matrice_relation.GetLength(0); i++)
            {
                ...
                ListeAdjacence[matrice_relation[i, 0]-1].Add(matrice_relation[i, 1]);       /// On ajoute à la Liste d'adjacence du nombre de la colonne de gauche celui de la colonne de droite de la matrice_relation
                ListeAdjacence[matrice_relation[i, 1]-1].Add(matrice_relation[i, 0]);       /// On fait également l'inverse car le graphe est non-orienté donc les relations sont réciproques
            }
        ...
        public int[,] GenererMatriceAdjacence(int[,] matrice_relation, int ordre)
        {
            ...
            for(int i = 0; i<matrice_relation.GetLength(0); i++)
            {
                MatriceAdjacence[matrice_relation[i, 0]-1, matrice_relation[i, 1]-1] = 1;     /// on rempli la matrice grâce à la matrice relation : on met des 1 lorsque les sommets i et j sont voisins.
                MatriceAdjacence[matrice_relation[i, 1]-1, matrice_relation[i, 0]-1] = 1;
            }
        ...
        public List<int> ParcoursEnProfondeurDabord(Noeud[] noeuds)
        {
            ...
            while (SommetDepart>34 || SommetDepart<=0)
            {
        ...
        public List<int> ParcoursEnLargeurDabord(Noeud[] noeuds)
        {
